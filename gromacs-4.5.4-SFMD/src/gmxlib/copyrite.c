                   ...
                   int retsize, int *cqnum)
{
  ...
  if (be_cool() && ((fp = low_libopen(db, FALSE)) != NULL))
  {
    ...
    int nlines = 0;
    //
    /* 1) First pass: count non-empty lines */
    ...
    if (nlines <= 0)
    {
      ...
      retstring[retsize - 1] = '\0';
      return;
    }
    /* 2) Pick a safe random index in [0, nlines-1] */
    {
      int seed = time(NULL);   /* keep old-style reseed if you want */
      double u = rando(&seed); /* nominally in [0,1) */
      int idx = (int)(u * nlines);
      if (idx >= nlines)
        idx = nlines - 1; /* guard FP edge or bad RNG */
      /* 3) Second pass: rewind and fetch that line */
      rewind(fp);
      int cur = -1;
      while (fgets(buf, (int)sizeof(buf), fp) != NULL)
      {
        ...
        if (cur == idx)
        {
          /* strip trailing LF/CR */
          size_t len = strlen(buf);
          if (len && buf[len - 1] == '\n')
            buf[--len] = '\0';
          if (len && buf[len - 1] == '\r')
            buf[--len] = '\0';
          /* clamp overly long encoded line */
          if ((int)len >= STRLEN)
            buf[STRLEN - 1] = '\0';

          /* 4) copy encoded line, then decode (~byte) */
          strncpy(retstring, buf, retsize);
          retstring[retsize - 1] = '\0';
          f(retstring);
          break;
        }
...
#define NGPL (int)asize(GPLText)
  ...
  ster_print(out, "A modified version of GROMACS 4.5.4");
  fprintf(out, "\n");
