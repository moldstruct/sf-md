...
static LaserPulse init_laser_pulse(const t_inputrec *ir, gmx_rng_t rng)
{
    ...
    /* Read user inputs */
    p.t0_ps = (double)ir->userreal1;
    p.pulse_energy_J = (double)ir->userreal2;
    p.sigma_ps = (double)ir->userreal3;
    p.spot_diameter_nm = (double)ir->userreal4;
    p.wavelength_nm = (double)ir->userreal5;
...
static void flush_charge_window(const t_commrec *cr, t_mdatoms *mdatoms,
                                real *charge_buf, real *write_buf,
                                int interval, int n_valid, FILE *fcharges)
{
    int i, w;
    ...
    if (MASTER(cr) && fcharges != NULL)
    {
        for (w = 0; w < n_valid; w++)
        {
            ...
            {
                write_buf[w * mdatoms->nr + i] = charge_buf[i * interval + w];
            }
        }

        fwrite(write_buf, sizeof(write_buf[0]),
              (size_t)mdatoms->nr * n_valid, fcharges);
    }
}
                                      ...
                                      gmx_rng_t rng)
{
    ...
    {
        int w = (int)(step % charge_log_interval);

        for (i = mdatoms->start; i < mdatoms->start + mdatoms->homenr; i++)
        {
            charge_buf[i * charge_log_interval + w] = mdatoms->chargeA[i];
        }

        if (w == charge_log_interval - 1)
        {
            flush_charge_window(cr, mdatoms, charge_buf, write_buf,
                                charge_log_interval, charge_log_interval, fcharges);
        }
    }
             ...
             gmx_runtime_t *runtime)
{
    ...
    FILE *fcharges = NULL;
    gmx_rng_t ion_rng = NULL;
    /* Per-atom charge buffer for charges_over_time.bin: atom-major, stride
     * charge_log_interval reals per atom. Filled locally every step (no
     * communication); gathered across PD ranks and flushed to disk only
     * once per charge_log_interval steps, so the expensive cross-rank
     * ring-communication happens once per window instead of once per step,
     * while every single step's charge is still recorded. Allocated on
     * every rank (all ranks need to hold their own local slice); write_buf
     * (the transposed, time-major staging buffer for the actual fwrite) is
     * only needed on MASTER. */
    real *charge_buf = NULL;
    real *write_buf = NULL;
    int charge_log_interval = 1;
    ...
    if (bIonize)
    {
        /* Each rank/thread gets its own private RNG state (gmx_rng_make_seed()
         * pulls from /dev/random when available, so ranks/replicas launched
         * within the same wall-clock second still get independent streams).
         * This also avoids sharing mutable state across threads, which
         * matters because GROMACS thread-MPI runs multiple "ranks" as
         * threads inside one process -- the plain libc rand()/srand() this
         * used to call are not safe to share across those threads. */
        ion_rng = gmx_rng_init(gmx_rng_make_seed());

        laser = init_laser_pulse(ir, ion_rng);

        /* The carrier phase is a property of the physical laser field and
         * must be identical for every rank, even though each rank just drew
         * its own independent random value for it above. gmx_bcast() must
         * only be called when actually running in parallel: with -nt 1 no
         * thread-MPI environment is started (see mdrunner() in runner.c),
         * so cr->mpi_comm_mygroup is never set up and MPI_Bcast segfaults. */
        if (PAR(cr))
        {
            gmx_bcast(sizeof(laser.phase_rad), &laser.phase_rad, cr);
        }

        init_adk_precomputed_table(&adk_table);

        if (MASTER(cr))
        {
            print_laser_pulse_info(stderr, &laser, &adk_table);
            print_adk_precomputed_table(stderr, &adk_table);

            if (fplog != NULL)
            {
                print_laser_pulse_info(fplog, &laser, &adk_table);
                print_adk_precomputed_table(fplog, &adk_table);
            }
        }
    }
    else
    ...
    /* Only MASTER creates the shared output directory/files below -- every
     * rank doing this concurrently would race on mkdir()/fopen(FOO, "wb")
     * against the same paths. Non-master ranks keep fcharges == NULL.
     * charges_over_time.bin's contents used to only be MASTER's local PD
     * subset (see gromacs_vmc_pd_charge_logging_bug) -- now fixed via
     * flush_charge_window()'s cross-rank gather (move_reals_strided),
     * called from do_ionize_step()/below, so charge_buf must be allocated
     * on *every* rank (not just MASTER) even though only MASTER ever
     * fwrite()s it. masses.bin is unaffected: mdatoms->massT is static
     * per-topology data, already fully populated on every rank (unlike
     * chargeA, which only this rank's local atoms get updated during the
     * run), so that fwrite was already correct. */
    if (MASTER(cr))
    {
    ...
    if (bIonize)
    {
        fcharges = fopen("./simulation_output/charges_over_time.bin", "wb");
        if (fcharges == NULL)
        {
            printf("Error: could not open charges_over_time.bin for writing.\n");
            return 1;
        }
        ...
        if (fmasses == NULL)
        {
            printf("Error: could not open masses.bin for writing.\n");
            return 1;
        }
    ...
    /* charge_buf must exist on every rank (each rank buffers its own local
     * atoms every step); write_buf (the transpose staging buffer for the
     * actual fwrite) is only ever touched by MASTER inside
     * flush_charge_window(), so only allocate it there. */
    if (bIonize)
    {
        charge_log_interval = (ir->userint7 > 0) ? ir->userint7 : 1;

        snew(charge_buf, (size_t)mdatoms->nr * charge_log_interval);

        if (MASTER(cr))
        {
            snew(write_buf, (size_t)mdatoms->nr * charge_log_interval);
        }
    }
    ...
    { // THIS IS THE MAIN MD LOOP @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        userint7 - Charge-logging interval in steps: charges_over_time.bin/mean_charge_vs_time.txt
                   are gathered across ranks and flushed every userint7 steps instead of every
                   step (every step's charge is still buffered locally in the meantime, so no
                   time resolution is lost -- only the cross-rank communication is throttled).
                   (default = 1, i.e. every step, matching the old behavior's frequency)
        userint9 - Enable reading charges from file.

        The userreal are mostly the laser parameters
        userreal1 - pulse center / peak time [ps]
        userreal2 - pulse energy [J]
        userreal3 - pulse sigma [ps]
        userreal4 - focal spot diameter [nm]
        userreal5 - Wavelength [nm]
        userreal6 - autostop threshold
        */

        /* Ionize the atoms if necessary */
        if (bIonize)
        {
            IonizeStepStats ion_stats;
            int logging;
            ...
            if (bFirstStep)
            {
                if (ionize_first_step_init(mdatoms, ir->userint9, cr))
                {
                    if (fcharges != NULL)
                    {
                        fclose(fcharges);
            ...
            ion_stats = do_ionize_step(mdatoms,
                                       &laser,
                                       &adk_table,
                                       t,
                                       ir->delta_t,
            ...
            log_ionize_step(&ion_stats, t, logging, cr);

            /* do_ionize_step() only updates this rank's own home range of
             * mdatoms->chargeA -- under PD every rank keeps a full-length
             * array, but nothing keeps the other ranks' slices in sync
             * during the run itself (only the output-flush path re-syncs,
             * via the same primitive below, and only every charge_log_interval
             * steps). Charges here change every step while the pulse is on,
             * so without this, do_force() below would compute every
             * cross-rank pair (~(nnodes-1)/nnodes of all pairs) against a
             * stale/zero charge for the far atom -- both the reported
             * energies and the actual forces driving the dynamics would be
             * wrong. Re-sync the full array across ranks before do_force()
             * runs for this step. */
            if (PAR(cr) && !DOMAINDECOMP(cr))
            {
                move_reals_strided(cr, FALSE, GMX_LEFT, GMX_RIGHT,
    ...
    if (bIonize)
    {
        /* Flush any steps buffered since the last charge_log_interval
         * boundary (0 if nsteps divided evenly -- that window was already
         * flushed from inside the loop, and re-flushing here would
         * duplicate it in charges_over_time.bin). */
        int w_last = (int)(step % charge_log_interval);

        if (w_last != charge_log_interval - 1)
        {
            flush_charge_window(cr, mdatoms, charge_buf, write_buf,
                                charge_log_interval, w_last + 1, fcharges);
        }

        /* charges.txt used to index mdatoms->chargeA[0..natoms-1] as if it
         * held every atom in the system, which was only true for
         * single-rank runs (see gromacs_vmc_pd_charge_logging_bug) -- under
         * PD, everything past this rank's local [start, start+homenr) slice
         * was actually still the original pre-ionization topology charge,
         * not zero/garbage, since nothing had ever overwritten it. Gather
         * the true full-system array (collective -- every rank, not just
         * MASTER, must participate) before the MASTER-only dump below. */
        if (PAR(cr) && !DOMAINDECOMP(cr))
        {
            move_reals_strided(cr, FALSE, GMX_LEFT, GMX_RIGHT,
                               mdatoms->chargeA, 1,
