...
/* Seed for the ADK-tunnelling RNG.
 *
 * By default this is gmx_rng_make_seed(), which pulls from /dev/random, so
 * every run is an independent realisation - which is what you want for
 * ensemble statistics, and why two runs of the byte-identical system diverge
 * almost immediately once ionization starts.
 *
 * Setting GMX_SFMD_SEED pins it instead, making a run reproducible and so
 * making the ionization path testable at all: without it there is no way to
 * tell a real change in behaviour from the RNG simply landing differently.
 *
 * Each rank still gets its own stream - the ranks must not draw identical
 * ionization sequences - so the rank index is mixed in rather than every rank
 * using the same value.  A pinned run is therefore reproducible for a given
 * rank count, but the same seed at -nt 1 and at -nt 4 are different
 * realisations, which is unavoidable: the ranks own different atoms. */
             ...
             gmx_runtime_t *runtime)
{
    ...
    /* MolDStruct strong-field ionization; NULL without -ionize. */
    t_sfionize *sfion = NULL;
    ...
    if (bIonize)
    {
        sfion = sfionize_init(fplog, ir, mdatoms, cr);
    }
    ...
    { // THIS IS THE MAIN MD LOOP @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        Until 2026-08-23 these were the generic userint/userreal slots, and
        this block also documented userint2 (charge transfer), userint4 (read
        electronic states) and userint6 (collisional ionization) - none of
        which this build has ever read.  They were copied from the sibling
        gromacs-mc docs.
        */

        /* Ionize the atoms if necessary.  Must run before do_force(), so
         * that this step's forces see this step's charges. */
        sfionize_step(sfion, ir, mdatoms, t, step, bFirstStep, cr);
    ...
    /* End of main MD loop */
    sfionize_done(sfion, mdatoms, top_global->natoms, cr);
